...
def get_grammar():
    ...
    expr, arith, boolean, term, factor, atom = G.NonTerminals('<expr> <arith> <boolean> <term> <factor> <atom>')
    ...
    # <boolean>
    boolean %= expr + minor + expr, lambda h,s: MinorNode(s[1], s[3])
    boolean %= expr + minor_eq + expr, lambda h,s: MinorEqualNode(s[1], s[3])
    ...
    atom %= _not + expr, lambda h,s: NotNode(s[2])
    
    atom %= func_call, lambda h,s: s[1]
    atom %= new + idx, lambda h,s: InstantiateNode(s[2])
    atom %= isvoid + expr, lambda h,s: IsVoidNode(s[2])
    atom %= complement + expr, lambda h,s: ComplementNode(s[2])

    atom %= idx + left_arrow + expr, lambda h,s: AssignNode(s[1], s[3])
    ...
    # <let-expr>
    let_expr %= let + let_att + _in + expr, lambda h,s: LetNode(s[2], s[4])
    ...
    func_call %= expr + dot + idx + opar + arg_list + cpar, lambda h,s: DispatchNode(s[1], s[3], s[5])
    func_call %= expr + at + idx + dot + idx + opar + arg_list + cpar, lambda h,s: DispatchNode(s[1], s[5], s[7], s[3])
